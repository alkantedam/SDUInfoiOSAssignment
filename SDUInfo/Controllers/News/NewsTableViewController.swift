...
class NewsTableViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell{
        let cell = newsTableView.dequeueReusableCell(withIdentifier: "newsCell", for: indexPath) as! NewsTableTableViewCell
        cell.newsTitleLabel.text = news[indexPath.row].newsTitle
        cell.newsImage.image = news[indexPath.row].newsImage
        cell.newsTitleLabel.numberOfLines = 4
        print("Heloo")
        return cell
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "toNewsDetailSegue"{
            ...
            newsDetailVC.newsDetailTitle = news[(newsTableView.indexPathForSelectedRow?.row)!].newsTitle
            newsDetailVC.newsDetailFull = news[(newsTableView.indexPathForSelectedRow?.row)!].newsFull
            newsDetailVC.newsDetailImage = news[(newsTableView.indexPathForSelectedRow?.row)!].newsImage
